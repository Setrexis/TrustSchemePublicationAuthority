package eu.lightest.tspa.services;
...
import eu.lightest.tspa.api.VersionFactory;
import eu.lightest.tspa.api.v1.publication.misc.Base16;
...
public class PublicationService {
    ...
    public Response test() {
        ...
        return Response.ok("ApiVersion: " + mApiVersion).build();
    }

   @Path("/{scheme_name}/trust-list")
   @Consumes(MediaType.APPLICATION_JSON)
   @PUT
   public Response publishTrustListJson(@PathParam("scheme_name") String schemeName, String data) {
       ...
       mLog.debug("Calling publishTrustListJson endpoint -> publishing trust list from JSON data!");
       ...
       try {
           ...
           return Response.serverError().entity("Failed to publish trust list! " + e.getMessage()).build();
    ...
    @PUT
    public Response publishTrustListXml(@PathParam("scheme_name") String schemeName, String data) {

        mLog.info("--------------- PUBLISH TRUST LIST (XML) ---------------");
        mLog.debug("Calling publishTrustListXML endpoint -> publishing trust list from XML data!");
        mLog.info("Trust Scheme Name: '" + schemeName +"'");
        mLog.info("Data: '"+ data + "'");
        ...
        try {
            ...
            pub.setUrl("https://tspa-unhcr.tug.do.nlnetlabs.nl/tspa/api/v1/scheme/" + schemeName);
   ...
   @PUT
   public Response publishTrustListUrl(@PathParam("scheme_name") String schemeName, String url){
    ...
    public Response deleteTrustList(@PathParam("scheme_name") String schemeName) {
        ...
        try {
            ...
            switch(e.getCode()) {
                case 404:
                    return Response.status(e.getCode()).entity(e.getCode() + ": There exists No trust list with the given trust scheme parameter" ).build();
                case 500:
                    return Response.status(e.getCode()).entity("ZoneManager is currently not reachable!").build();
                default:
                    return Response.status(e.getCode()).entity(e.getCode() + ": " + e.getMessage()).build();
            }
        }
   ...
   @Consumes(MediaType.APPLICATION_JSON)
   @PUT
    public Response publishSchemeJson(@PathParam("scheme_name") String schemeName, String data) {
       ...
       try {
           ...
           mLog.error(e.getMessage(), e);
           return Response.serverError().entity("Failed to publish trust scheme " + schemeName + " : " + e.getMessage()).build();
   ...
   @Consumes(MediaType.TEXT_PLAIN)
   @PUT
   public Response publishSchemeUrl(@PathParam("scheme_name") String schemeName, String url) {
       ...
       try {
           ...
           mLog.error(e.getMessage(), e);
           return Response.serverError().entity("Failed to publish trust scheme " + schemeName).build();
       }
    ...
    public Response deleteScheme(@PathParam("scheme_name") String schemeName) {
        ...
        try {
            ...
            switch(e.getCode()) {
                case 404:
                    return Response.status(e.getCode()).entity(e.getCode() + ": There exists No trust scheme with the given trust scheme parameter" ).build();
                case 500:
                    return Response.status(e.getCode()).entity("ZoneManager is currently not reachable!").build();
                default:
                    return Response.status(e.getCode()).entity(e.getCode() + ": " + e.getMessage()).build();
